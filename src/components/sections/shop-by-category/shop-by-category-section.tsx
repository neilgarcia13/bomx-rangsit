        ...
        <div className="grid gap-4 md:grid-cols-2 lg:auto-rows-56 lg:grid-cols-12">
          {categories.map((category, index) => (
            <ScrollReveal
              ...
              delay={index * 0.07}
              className={cn(
                category.featured ? "md:col-span-2 lg:col-span-6 lg:row-span-2" : "lg:col-span-3",
              )}
            >
              <Link
                href={`/products?category=${category.slug}`}
                className={cn(
                  "group/category border-border bg-muted focus-visible:ring-ring focus-visible:ring-offset-card relative isolate block h-full overflow-hidden rounded-xl border focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none",
                  category.featured ? "min-h-112 md:min-h-120 lg:min-h-0" : "min-h-64 lg:min-h-0",
                )}
              >
